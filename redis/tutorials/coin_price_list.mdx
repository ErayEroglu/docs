...
In this tutorial, we will develop a simple coin price list using the GraphQL API
of Upstash. You can call the application `backendless` because we will access the
...
code. The use case involves publicly available read-only data for web applications
where you need low latency. The data is updated frequently by another backend
application; you want your users to see the latest updated data. Examples:
Leaderboards, news lists, blog lists, product lists, top N items on homepages.

### `1` Project Setup:

Create a Next application: `npx create-next-app`.

Install the Apollo GraphQL client: `npm i @apollo/client`
...
In the Upstash console, copy the read-only access key on your API configuration
page (GraphQL Explorer > Configure API). In the `_app.js`, create the Apollo
client and replace your access key as below:

<Warning>
  You need to use a Read-Only Access Key, because the key will be accessible
  publicly.
</Warning>
...
Go to [http://localhost:3000/](http://localhost:3000/) ð